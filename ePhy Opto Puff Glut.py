...
folder = r"C:\Users\Angel.BAUDON\Desktop\Opto puff\OptoPuff Glut"
...
for sub_folder in glob.glob(rf'{folder}\*/'):
    ...
    for cell in cells:
        ...
        post_stim = [x for x in data[620*sampling:] if str(x) != 'nan']
        sg = savgol_filter(post_stim, 101, 3)
        deriv = np.diff(sg)
        
        max_deriv_index = np.where(deriv == np.nanmax(deriv))[0][0]
        max_amp = np.nanmax(sg[max_deriv_index:])
        max_index = np.where(sg == max_amp)[0][0]+620*sampling
        
        # plt.figure()
        # plt.subplot(211)
        # plt.plot(post_stim), plt.plot(sg)
        # plt.plot(np.where(sg == max_amp)[0][0], deriv[np.where(sg == max_amp)[0][0]], 'og')
        # plt.subplot(212)
        # plt.plot(deriv), plt.plot(max_deriv_index, deriv[max_deriv_index], 'og')
